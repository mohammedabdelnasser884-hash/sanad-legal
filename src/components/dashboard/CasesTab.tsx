...
function CasesTab({ cases, casesFilter, setCasesFilter, casesPage, setCasesPage, casesTotal, casesLoading, fetchCases, setShowCaseModal, setSelectedCase, loadingCases, dbError }: any) {
    ...
    return React.createElement('div', { className: "space-y-4 fade-in" },
        ...
        React.createElement('div', { className: "flex items-center justify-between gap-4" },
            React.createElement('h3', { className: "text-sm font-black text-white" }, "منظومة القضايا"),
            React.createElement('button', { onClick: () => setShowCaseModal(true), className: "flex items-center bg-gradient-to-tr from-premium-gold to-amber-200 text-premium-bg px-3 py-2 rounded-xl text-xs font-black shadow-lg gap-1 active:scale-95 transition-transform" },
        ...
        React.createElement('div', { className: "flex items-center bg-white/5 rounded-2xl p-1 gap-1" },
            caseSections.map(s => {
                ...
                return React.createElement('button', {
                    key: s.key,
                    onClick: () => { setCasesFilter(s.key); setCasesPage(0); fetchCases(0, s.key); },
            ...
            : dbError
                ...
                : cases.length === 0
                    ? React.createElement('div', { className: "bg-premium-card border border-white/5 rounded-2xl px-5 py-10 text-center space-y-2" },
                        React.createElement('p', { className: "text-3xl mb-1" }, activeSection.emoji),
                    ...
                    : React.createElement('div', { className: "space-y-2" },
                        ...
                        casesTotal > PAGE_SIZE && React.createElement('div', { className: "flex items-center justify-between gap-2 pt-1" },
                            React.createElement('button', {
                                onClick: () => { const p = casesPage - 1; setCasesPage(p); fetchCases(p, casesFilter); },
                                disabled: casesPage === 0 || casesLoading,
                            ...
                            React.createElement('span', { className: "text-[10px] text-slate-500 font-black whitespace-nowrap" },
                                `${casesPage * PAGE_SIZE + 1}–${Math.min((casesPage + 1) * PAGE_SIZE, casesTotal)} / ${casesTotal}`
                            ),
                            React.createElement('button', {
                                onClick: () => { const p = casesPage + 1; setCasesPage(p); fetchCases(p, casesFilter); },
                                disabled: (casesPage + 1) * PAGE_SIZE >= casesTotal || casesLoading,
                                className: "flex-1 py-2.5 rounded-xl text-xs font-black active:scale-[0.98] transition-all disabled:opacity-30",
                                style: { background: 'rgba(212,175,55,0.07)', border: '1px solid rgba(212,175,55,0.18)', color: '#D4AF37' }
                            }, "التالي ←")
                        )
