...
function ArchiveTab({cases, clients}){
    ...
    const fetchDocs = async () => {
        setLoading(true);
        const {data, error} = await db.from('case_documents')
            .select('*')
            .order('created_at', {ascending: false});
        if (!error) setAllDocs(data || []);
        setLoading(false);
    };
    ...
    const handleFileSelect = e => {
        ...
        if (!f) return;
        // ⚠️ فحص نوع وحجم الملف قبل القبول — راجع validateUploadFile في utils.ts.
    ...
    const handleUpload = async () => {
        ...
        // فحص دفاعي ثاني قبل الرفع الفعلي
    ...
    const handleDelete = async (doc) => {
        ...
        if (storageErr) {
            setDeletingId(null);
            toast('❌ فشل حذف الملف من التخزين', true);
            return;
        }
    ...
    let filtered = allDocs.filter(d => {
        ...
        if (q) {
            const matchDoc = (d.file_name||'').toLowerCase().includes(q) || (d.category||'').toLowerCase().includes(q);
            const matchCase = cases.find(c => c.id === d.case_id && (c.title||'').toLowerCase().includes(q));
            const matchClient = cases.find(c => c.id === d.case_id && clients.find(cl => cl.id === c.client_id && (cl.full_name||'').toLowerCase().includes(q)));
            if (!matchDoc && !matchCase && !matchClient) return false;
        }
        if (filterCat !== 'الكل' && d.category !== filterCat) return false;
        if (filterCase && d.case_id !== filterCase) return false;
        return true;
    });
    ...
    return React.createElement('div',{className:"space-y-4 fade-in"},
        ...
        React.createElement('div',{className:"flex items-center justify-between"},
            React.createElement('div',null,
                ...
                React.createElement('p',{className:"text-[9px] text-slate-500 mt-0.5"},allDocs.length+" مستند · "+sizeStr)
            ),
        ...
        showForm && pendingFile && React.createElement('div',{className:"bg-premium-card border border-purple-500/20 rounded-2xl p-4 space-y-3 slide-up"},
            React.createElement('div',{className:"flex items-center gap-3 p-3 bg-premium-bg rounded-xl"},
                React.createElement('div',{className:"w-10 h-10 rounded-xl flex items-center justify-center text-2xl shrink-0 "+getDocMeta(pendingFile).bg},
                    getDocMeta(pendingFile).emoji
                ),
                ...
                React.createElement('button',{
                    onClick:()=>{setShowForm(false);setPendingFile(null);if(fileInputRef.current)fileInputRef.current.value='';},
            ...
            React.createElement(Sel,{
                label:"تصنيف المستند",value:docCategory,
                onChange:e=>setDocCategory(e.target.value),
                options:['مذكرة دفاع','صحيفة دعوى','حكم قضائي','عقد','توكيل','مستند رسمي','صورة','أخرى']
            }),
            cases.length>0&&React.createElement(Sel,{
                label:"ربط بقضية (اختياري)",value:docCaseId,
                onChange:e=>setDocCaseId(e.target.value),
                options:[{value:'',label:'— غير مرتبط بقضية —'},...cases.map(c=>({value:c.id,label:c.title}))]
            }),
            React.createElement('div',{className:"flex gap-2"},
                React.createElement('button',{
                    onClick:handleUpload, disabled:uploadingDoc,
                    className:"flex-1 py-2.5 bg-gradient-to-tr from-purple-600 to-purple-400 text-white rounded-xl text-xs font-black flex items-center justify-center gap-1.5 disabled:opacity-50 active:scale-95"
                },uploadingDoc?React.createElement(React.Fragment,null,React.createElement(I.Spin),"جاري الرفع..."):React.createElement(React.Fragment,null,"☁️ رفع للأرشيف السحابي")),
        ...
        React.createElement('div',{className:"space-y-2.5"},
            ...
            React.createElement('div',{className:"relative"},
                ...
                React.createElement('input',{
                    ...
                    placeholder:"ابحث في المستندات والقضايا والموكلين...",
            ...
            React.createElement('div',{className:"flex items-center justify-between"},
                React.createElement('span',{className:"text-[9px] text-slate-500 font-bold"},
                    searchQ||filterCat!=='الكل'
                        ? filtered.length+" نتيجة من "+allDocs.length
                        : allDocs.length+" مستند في الأرشيف"
                ),
        ...
        loading
            ...
            : allDocs.length === 0
                ? React.createElement('div',{className:"text-center py-16 space-y-4"},
                    ...
                    React.createElement('p',{className:"text-white font-black text-sm"},"الأرشيف فارغ"),
                    React.createElement('p',{className:"text-slate-500 text-xs"},"ارفع أول مستند بالضغط على زر رفع مستند"),
                    React.createElement('button',{
                        onClick:()=>fileInputRef.current&&fileInputRef.current.click(),
                        className:"mx-auto mt-2 flex items-center gap-2 px-4 py-2.5 bg-purple-500/10 border border-purple-500/20 text-purple-300 rounded-xl text-xs font-black active:scale-95"
                    },React.createElement(I.Plus),"ابدأ الأرشفة")
                  )
                : filtered.length === 0
                    ? React.createElement('div',{className:"text-center py-12 space-y-3"},
                        React.createElement('div',{className:"text-3xl mx-auto w-fit"},"🔍"),
                        React.createElement('p',{className:"text-slate-400 font-bold text-xs"},"لا توجد نتائج لبحثك"),
                        React.createElement('button',{onClick:()=>{setSearchQ('');setFilterCat('الكل');},className:"text-purple-400 text-xs font-bold"},"مسح الفلاتر")
                      )
                    : React.createElement('div',{className:"space-y-3"},
                        filtered.map(doc => {
                            const {emoji, bg, canPreview, isPdf} = getDocMeta(doc);
                            ...
                            return React.createElement('div',{key:doc.id, className:"bg-premium-card border border-white/5 rounded-xl px-3 py-2.5 flex items-center gap-2.5 hover:border-purple-500/20 transition-all"},
                                ...
                                React.createElement('div',{
                                    className:`w-9 h-9 rounded-lg border flex items-center justify-center text-lg shrink-0 ${canPreview?'cursor-pointer active:scale-90':''} ${bg}`,
                                    onClick:()=>canPreview&&setViewingDoc(doc)
                                }, emoji),
                                // المعلومات
                                React.createElement('div',{className:"flex-1 min-w-0"},
                                ...
                                React.createElement('div',{className:"flex items-center gap-1 shrink-0"},
                                    canPreview && React.createElement('button',{
                                        ...
                                        className:"w-7 h-7 rounded-lg bg-purple-500/10 border border-purple-500/15 flex items-center justify-center text-purple-400 active:scale-90 text-sm"
                                    },"👁"),
                                    React.createElement('a',{
                                        href:doc.file_url, target:'_blank', rel:'noreferrer',
                                        className:"w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 active:scale-90"
                                    }, React.createElement(I.Download,{className:"w-3.5 h-3.5"})),
                                    React.createElement('button',{
                                        onClick:()=>handleDelete(doc),
                                        disabled:deletingId===doc.id,
                                        className:"w-7 h-7 rounded-lg bg-rose-500/5 border border-rose-500/10 flex items-center justify-center text-rose-400/60 hover:text-rose-400 active:scale-90 disabled:opacity-40"
                                    }, deletingId===doc.id ? React.createElement(I.Spin) : React.createElement(I.Trash,{className:"w-3.5 h-3.5"}))
                                )
                            );
                        })
                      )
