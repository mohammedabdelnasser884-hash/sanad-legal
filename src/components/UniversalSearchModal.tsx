...
function UniversalSearchModal({cases, clients, db, onClose, onOpenCase, onOpenClient}){
    ...
    const [activeFilter, setActiveFilter] = useState('all'); // all | cases | clients | sessions | notes | docs
    const inputRef = useRef(null);

    useEffect(()=>{
        inputRef.current?.focus();
        Promise.all([
            db.from('case_documents').select('id,case_id,file_name,category,created_at').order('created_at', { ascending: false }).limit(500),
            db.from('case_sessions').select('id,case_id,session_date,description,result,next_action').order('session_date', { ascending: false }).limit(500),
            db.from('case_notes').select('id,case_id,content,created_at').order('created_at', { ascending: false }).limit(500),
        ]).then(([{data:docs},{data:sessions},{data:notes}])=>{
            setAllDocs(docs||[]);
    ...
    // دالة تحويل رقم القيد للبحث بكلا الشكلين
    ...
    const matchedCases = query.length >= 1 ? cases.filter(c =>
    ...
    const matchedClients = query.length >= 1 ? clients.filter(c =>
    ...
    const matchedDocs = query.length >= 1 ? allDocs.filter(d =>
        (d.file_name||'').toLowerCase().includes(query) ||
        (d.category||'').toLowerCase().includes(query)
    ) : [];

    const matchedSessions = query.length >= 1 ? allSessions.filter(s =>
        (s.description||'').toLowerCase().includes(query) ||
        (s.result||'').toLowerCase().includes(query) ||
        (s.next_action||'').toLowerCase().includes(query) ||
        (s.session_date||'').includes(query)
    ) : [];

    const matchedNotes = query.length >= 1 ? allNotes.filter(n =>
    ...
    const totalResults =
        ...
        (show.sessions ? matchedSessions.length : 0) +
        (show.notes    ? matchedNotes.length    : 0) +
        (show.docs     ? matchedDocs.length     : 0);
    ...
    return React.createElement('div',{className:"fixed inset-0 z-[60] bg-black/90 backdrop-blur-md flex flex-col fade-in"},
        ...
        React.createElement('div',{className:"px-4 pt-safe pt-4 pb-3 border-b border-white/10 bg-premium-card/80 backdrop-blur-lg shrink-0"},
            React.createElement('div',{className:"flex items-center gap-3"},
                React.createElement('div',{className:"relative flex-1"},
                    React.createElement('span',{className:"absolute right-3 top-1/2 -translate-y-1/2 text-premium-gold"},React.createElement(I.Search)),
            ...
            // فلاتر سريعة
            query.length > 0 && React.createElement('div',{className:"flex gap-1.5 mt-3 overflow-x-auto no-scrollbar pb-0.5"},
                [
                    ...
                    {key:'sessions',label:'🗓 الجلسات',  count: matchedSessions.length},
                    {key:'notes',   label:'📝 الملاحظات',count: matchedNotes.length},
                    {key:'docs',    label:'📁 المستندات',count: matchedDocs.length},
                ...
                }, f.label+(f.count>0?' ('+f.count+')':'')) : null)
            ),
            query.length > 0 && !React.Children.toArray(null).length && React.createElement('p',{className:"text-[10px] text-slate-500 mt-2 font-bold"},
                hasResults ? totalResults+" نتيجة" : "لا توجد نتائج"
            )
        ),
        ...
        React.createElement('div',{className:"flex-1 overflow-y-auto no-scrollbar p-4 space-y-5"},
            ...
            // حالة البداية
            !query && React.createElement('div',{className:"text-center py-16 space-y-4"},
                React.createElement('div',{className:"text-5xl"},"🔍"),
                React.createElement('p',{className:"text-white font-black"},"البحث الشامل"),
                React.createElement('p',{className:"text-slate-500 text-xs leading-relaxed"},"ابحث في القضايا والجلسات والملاحظات والمستندات"),
                React.createElement('div',{className:"flex justify-center gap-2 flex-wrap mt-4"},
                    ['رقم القضية','اسم الموكل','اسم المحكمة','نوع القضية','اسم المستند','وصف جلسة'].map(hint=>
                        React.createElement('button',{key:hint,onClick:()=>setQ(hint),className:"text-[10px] px-3 py-1.5 bg-white/5 border border-white/10 text-slate-400 rounded-xl hover:bg-white/10 active:scale-95"},hint)
                    )
                )
            ),

            // القضايا
            show.cases && matchedCases.length > 0 && React.createElement('div',{className:"space-y-1.5"},
            ...
            // الموكلون
            show.clients && matchedClients.length > 0 && React.createElement('div',{className:"space-y-3"},
                React.createElement('div',{className:"flex items-center gap-2"},
            ...
            // الجلسات
            show.sessions && matchedSessions.length > 0 && React.createElement('div',{className:"space-y-3"},
                React.createElement('div',{className:"flex items-center gap-2"},
                    ...
                    React.createElement('span',{className:"text-[9px] text-slate-500"},matchedSessions.length+" نتيجة")
                ),
            ...
            // الملاحظات
            show.notes && matchedNotes.length > 0 && React.createElement('div',{className:"space-y-3"},
                React.createElement('div',{className:"flex items-center gap-2"},
                    ...
                    React.createElement('span',{className:"text-[9px] text-slate-500"},matchedNotes.length+" نتيجة")
                ),
                matchedNotes.map(n => {
            ...
            // المستندات
            show.docs && matchedDocs.length > 0 && React.createElement('div',{className:"space-y-3"},
                React.createElement('div',{className:"flex items-center gap-2"},
                    ...
                    React.createElement('span',{className:"text-[9px] text-slate-500"},matchedDocs.length+" نتيجة")
                ),
                matchedDocs.map(doc => {
            ...
            // لا نتائج
            query && !hasResults && !loadingExtra && React.createElement('div',{className:"text-center py-16 space-y-3"},
