...
function SettingsPage({profile, isAdmin, country, onCountryChange, onClose}){
  ...
  const [tgDailyToken, setTgDailyToken] = useState('');
  const [tgDailyChat,  setTgDailyChat]  = useState('');
  const [tgInstantToken, setTgInstantToken] = useState('');
  ...
  useEffect(()=>{
    if(section==='notifications' && !tgLoaded){
      Promise.all([
        loadOfficeSetting('tg_daily_token'),
        loadOfficeSetting('tg_daily_chat'),
        loadOfficeSetting('tg_instant_token'),
        loadOfficeSetting('tg_instant_chat'),
      ]).then(([dt,dc,it,ic])=>{
        setTgDailyToken(dt||''); setTgDailyChat(dc||'');
        setTgInstantToken(it||''); setTgInstantChat(ic||'');
        setTgLoaded(true);
      });
    }
  ...
  const saveTg = async () => {
    ...
    try {
      await Promise.all([
        saveOfficeSetting('tg_daily_token', tgDailyToken.trim()),
        saveOfficeSetting('tg_daily_chat',  tgDailyChat.trim()),
        saveOfficeSetting('tg_instant_token', tgInstantToken.trim()),
  ...
  const handleLogoUpload = (e: any) => {
    ...
    reader.onload = (ev) => office.setLogoPreview(ev.target?.result as string);
          ...
          React.createElement('div',{className:"flex items-center gap-3"},
            ...
            React.createElement('div',{className:"flex-1 space-y-2"},
              ...
              (office.logoPreview || office.officeSettings.logoUrl) && React.createElement('button',{
                onClick:()=>{ office.setLogoFile(null); office.setLogoPreview(null); office.setOfficeSettings(s=>({...s, logoUrl:''})); },
          ...
          ...[
            ...
            React.createElement('div',{key:key},
              ...
              React.createElement('input',{
                value:(office.officeSettings as any)[key] || '',
                onChange:(e:any)=>office.setOfficeSettings((s:any)=>({...s,[key]:e.target.value})),
                placeholder,
      ...
      section==='notifications'&&isAdmin&&React.createElement('div',{className:"space-y-4 fade-in"},
        ...
        React.createElement('div',{className:"bg-premium-card border border-white/5 rounded-2xl p-4 space-y-4"},
          ...
          React.createElement('div',null,
            ...
            React.createElement('div',{className:"relative"},
              React.createElement('input',{
                type:showTgDailyToken?"text":"password", value:tgDailyToken,
                onChange:(e:any)=>setTgDailyToken(e.target.value),
              ...
              React.createElement('button',{onClick:()=>setShowTgDailyToken(v=>!v),className:"absolute left-3 top-3 text-slate-500 text-xs"},showTgDailyToken?'🙈':'👁')
            )
          ),
          React.createElement('div',null,
            ...
            React.createElement('input',{
              ...
              onChange:(e:any)=>setTgDailyChat(e.target.value),
        ...
        React.createElement('div',{className:"bg-premium-card border border-white/5 rounded-2xl p-4 space-y-4"},
          ...
          React.createElement('div',null,
            ...
            React.createElement('div',{className:"relative"},
              React.createElement('input',{
                type:showTgInstantToken?"text":"password", value:tgInstantToken,
                onChange:(e:any)=>setTgInstantToken(e.target.value),
                placeholder:"123456789:ABCdefGHIjklMNOpqrSTUvwxYZ",
                className:"w-full p-3 text-xs rounded-xl border border-white/10 bg-premium-bg text-white placeholder-slate-600",
                style:{fontFamily:'monospace',direction:'ltr',textAlign:'left'}
              }),
              React.createElement('button',{onClick:()=>setShowTgInstantToken(v=>!v),className:"absolute left-3 top-3 text-slate-500 text-xs"},showTgInstantToken?'🙈':'👁')
            )
          ),
          React.createElement('div',null,
            React.createElement('label',{className:"block text-[10px] font-black text-slate-400 mb-1.5"},"💬 Chat ID"),
            React.createElement('input',{
              ...
              onChange:(e:any)=>setTgInstantChat(e.target.value),
              placeholder:"-1001234567890",
              className:"w-full p-3 text-xs rounded-xl border border-white/10 bg-premium-bg text-white placeholder-slate-600",
              style:{fontFamily:'monospace',direction:'ltr',textAlign:'left'}
            })
        ...
        tgLoaded&&React.createElement('div',{className:"bg-premium-card border border-white/5 rounded-2xl p-4 space-y-3"},
          React.createElement('div',{className:"space-y-1.5"},
            ...
            React.createElement('div',{className:"flex items-center gap-2"},
              React.createElement('span',{className:`w-2 h-2 rounded-full ${tgDailyToken?'bg-emerald-400':'bg-rose-400'}`}),
              React.createElement('span',{className:"text-[10px] text-slate-300"},tgDailyToken?'Bot Token: محفوظ ✓':'غير مضبوط')
            ),
            React.createElement('div',{className:"flex items-center gap-2"},
          ...
          React.createElement('div',{className:"space-y-1.5 pt-2 border-t border-white/5"},
            ...
            React.createElement('div',{className:"flex items-center gap-2"},
              React.createElement('span',{className:`w-2 h-2 rounded-full ${tgInstantToken?'bg-emerald-400':'bg-rose-400'}`}),
              React.createElement('span',{className:"text-[10px] text-slate-300"},tgInstantToken?'Bot Token: محفوظ ✓':'غير مضبوط')
            ),
            React.createElement('div',{className:"flex items-center gap-2"},
