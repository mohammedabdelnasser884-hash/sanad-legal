...
import { loadOfficeSetting, saveOfficeSetting, COUNTRY_CONFIGS } from '../../constants';
...
- إذا لم تكن هذه المواد كافية وحدها للإجابة الكاملة على السؤال، وضّح ذلك صراحةً واستكمل بمعرفتك العامة مع التنبيه إلى ذلك.${confidenceLine}`;
    ...
    // BUG-06 FIX: كان بيكتب مباشرة على office_settings (select/update/insert
    // يدوي) بدل ما يستخدم saveOfficeSetting الموحّدة الموجودة أصلاً في
    // constants.ts — نفس مبدأ "مصدر واحد للحفظ" اللي اتطبق على SettingsPage.
    // saveOfficeSetting بتعمل نفس الـ select/update/insert وبتمسح الكاش
    // تلقائياً (مفيش داعي لـ invalidateOfficeCache يدوي بعد كده).
    const saveKey = async (k) => {
        const tenantId = profile?.tenant_id ?? null;
        if (!tenantId) { toast('❌ تعذر تحديد المكتب الحالي', true); return; }
        try {
            await saveOfficeSetting('groq_key', k);
            setHasKey(true);
