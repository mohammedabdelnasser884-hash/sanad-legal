...
export function useDashboardFeed(profile: any) {
    ...
    const fetchMissedSessions = useCallback(async () => {
        ...
        const { data: pastData } = await db.from('case_sessions')
            .select('id, session_date, session_time, description, case_id, result, next_action, title, case_number, court, case_type, circuit_number, plaintiff, plaintiff_role, defendant, defendant_role, cases(id,title,plaintiff,defendant,court_name,case_type,case_number_official,client_id)')
