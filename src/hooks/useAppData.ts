...
function buildNearestSessionMap(sessionsData: { case_id: string; session_date: string }[]): { [k: string]: string } {
    ...
    (sessionsData || []).forEach((s) => {
        ...
        if (s.session_date >= todayStr) {
            if (!upcoming[s.case_id] || s.session_date < upcoming[s.case_id]) {
                upcoming[s.case_id] = s.session_date;
            }
        } else {
            if (!latestPast[s.case_id] || s.session_date > latestPast[s.case_id]) {
                latestPast[s.case_id] = s.session_date;
            }
        }
    });
