...
describe('useAuthProfile', () => {
  ...
  it('جلسة موجودة لكن خطأ فعلي في جلب البروفايل (RLS/تكرار) → توست برسالة الخطأ، profile بيفضل null', async () => {
    getSessionResult = { data: { session: { user: USER } } };
    maybeSingleResult = { data: null, error: { message: 'duplicate row' } };
    const { result } = renderHook(() => useAuthProfile());
    await waitFor(() => expect(getSession).toHaveBeenCalled());
    await waitFor(() => expect(toast).toHaveBeenCalledWith('تعذر تحميل بيانات الحساب: duplicate row'));
    expect(result.current.profile).toBeNull();
  });
