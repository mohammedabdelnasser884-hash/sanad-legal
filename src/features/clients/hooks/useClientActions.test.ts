...
describe('useClientActions', () => {
  ...
  describe('handleSaveLawyer', () => {
    ...
    it('فشل (callAdminAction بترمي Error) → توست فشل برسالة الخطأ نفسها، من غير logActivity أو إغلاق المودال', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ البريد مستخدم بالفعل', true);
      expect(logActivity).not.toHaveBeenCalled();
    ...
    it('فشل باستثناء غير Error (مثلاً رمي قيمة عادية) → رسالة fallback عامة', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل إنشاء حساب المحامي، تحقق من الاتصال وأعد المحاولة', true);
    });
  });
});
