...
describe('useAdminUsers', () => {
  ...
  describe('handleAddUser', () => {
    ...
    it('فشل بـ Error → توست برسالة الخطأ نفسها، من غير logActivity/setShowAddUser(false)/fetchLawyers', async () => {
    ...
    it('فشل باستثناء غير Error → رسالة fallback عامة', async () => {
      callAdminAction.mockRejectedValue('some string error');
      const { result } = setup();
      await act(async () => {
        await result.current.handleAddUser({ full_name: 'سارة', email: 's@sanad.test', password: 'pass123', role: 'lawyer', permissions: {} });
      });
      expect(toast).toHaveBeenCalledWith('❌ فشل إنشاء الحساب، يرجى المحاولة مرة أخرى', true);
    });
  });
