...
describe('useAdminLegalLibrary', () => {
  ...
  describe('handleSaveLaw — إضافة قانون جديد (من غير ملف)', () => {
    ...
    it('فشل insert → توست بنص رسالة الخطأ، من غير toast نجاح أو logActivity أو fetchLaws', async () => {
      mockDb.setResult('laws:insert', { error: { message: 'duplicate key' } });
      const { result } = setup();
      await act(async () => { await result.current.handleSaveLaw(FORM, null); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ: duplicate key', true);
  ...
  describe('handleSaveLaw — تعديل قانون موجود', () => {
    ...
    it('فشل update → توست خطأ، من غير logActivity أو setEditingLaw(null)', async () => {
      mockDb.setResult('laws:update', { error: { message: 'x' } });
      const { result } = setup();
      act(() => { result.current.setEditingLaw(EXISTING); });
      await act(async () => { await result.current.handleSaveLaw(FORM, null); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ: x', true);
      expect(logActivity).not.toHaveBeenCalled();
  ...
  describe('handleSaveLaw — رفع ملف', () => {
    ...
    it('فشل الرفع (استثناء) → توست خطأ برسالة الرفع، من غير أي محاولة insert', async () => {
      mockDb.setResult('storage:upload', { reject: true });
      const { result } = setup();
      const file = makeFile('law.pdf', 1000);
      await act(async () => { await result.current.handleSaveLaw(FORM, file); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ: upload failed', true);
      expect(mockDb.insertSpy).not.toHaveBeenCalled();
    });
  });
  ...
  describe('handleProcessLaw', () => {
    ...
    it('extractErr مع context.json() → توست برسالة الـ body.error المستخرجة', async () => {
      ...
      const { result } = setup();
      await act(async () => { await result.current.handleProcessLaw(LAW); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في المعالجة: ملف PDF تالف', true);
    });
    ...
    it('extractErr مع context.text() بس (من غير json) → توست بنص الـ text المستخرج', async () => {
      ...
      const { result } = setup();
      await act(async () => { await result.current.handleProcessLaw(LAW); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في المعالجة: نص الخطأ من السيرفر', true);
    });

    it('extractErr من غير context خالص → رسالة عامة "حدث خطأ غير متوقع"', async () => {
      mockDb.setResult('functions:invoke', { data: null, error: { message: 'generic' } });
      const { result } = setup();
      await act(async () => { await result.current.handleProcessLaw(LAW); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في المعالجة: حدث خطأ غير متوقع', true);
    });

    it('extractData.error (من غير extractErr) → بيترمي برسالة extractData.error', async () => {
      mockDb.setResult('functions:invoke', { data: { error: 'قانون مكرر بالفعل' }, error: null });
      const { result } = setup();
      await act(async () => { await result.current.handleProcessLaw(LAW); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في المعالجة: قانون مكرر بالفعل', true);
    });
  ...
  describe('handleDeleteLaw', () => {
    ...
    it('فشل حذف السجل من الجدول (error) → توست خطأ، من غير logActivity أو تصفير confirmDeleteLaw', async () => {
      mockDb.setResult('laws:delete', { error: { message: 'fk violation' } });
      const { result } = setup();
      act(() => { result.current.setConfirmDeleteLaw(LAW_NO_FILE); });
      await act(async () => { await result.current.handleDeleteLaw(LAW_NO_FILE); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في الحذف: fk violation', true);
      expect(logActivity).not.toHaveBeenCalled();
    ...
    it('فشل حذف الملف من الـ storage (استثناء) → بيتلقط في catch، delete().eq() الجدول ما بيتناداش خالص', async () => {
      ...
      const { result } = setup();
      await act(async () => { await result.current.handleDeleteLaw(LAW_WITH_FILE); });
      expect(toast).toHaveBeenCalledWith('❌ خطأ في الحذف: remove failed', true);
