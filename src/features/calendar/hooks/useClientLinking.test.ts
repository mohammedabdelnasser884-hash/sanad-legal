...
describe('useClientLinking', () => {
  ...
  describe('handleLinkCase', () => {
    ...
    it('فشل إنشاء القضية (error) → توست فشل برسالة الخطأ، وقف فوري من غير onSaved أو بحث عن موكل', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل إنشاء القضية: insert failed', true);
      expect(onSaved).not.toHaveBeenCalled();
      expect(mockDb.clientsIlikeSpy).not.toHaveBeenCalled();
      expect(result.current.linkingCase).toBe(false);
    });
  ...
  describe('handleLinkExistingClient', () => {
    ...
    it('فشل الربط (error) → توست فشل، من غير تغيير clientStep', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل الربط: update failed', true);
      expect(result.current.clientStep).toBe('found');
    });
  ...
  describe('handleAddAndLinkClient', () => {
    ...
    it('فشل إضافة الموكل → توست فشل برسالة الخطأ، من غير أي محاولة ربط (مفيش UPDATE)', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل إضافة الموكل: client insert failed', true);
      expect(mockDb.casesUpdateSpy).not.toHaveBeenCalled();
    });

    it('الموكل اتضاف بنجاح لكن الربط فشل → توست فشل خاص بالربط، من غير clientStep=done', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل الربط: link failed', true);
  ...
  describe('handleAddClientOnly', () => {
    ...
    it('فشل الإدخال → توست فشل برسالة الخطأ', async () => {
      ...
      expect(toast).toHaveBeenCalledWith('❌ فشل إضافة الموكل: plain insert failed', true);
    });
