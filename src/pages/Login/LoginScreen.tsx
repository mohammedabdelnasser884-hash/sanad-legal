...
function LoginScreen({onLogin}: LoginScreenProps){
    ...
    const handleLogin=async(e: React.MouseEvent<HTMLButtonElement>)=>{
        ...
        if(error||data?.error||!data?.access_token){
            setLoading(false);
            setErr(data?.error || error?.message || 'تعذر تسجيل الدخول، تحقق من اتصالك بالإنترنت وحاول مرة أخرى');
            return;
        }
        ...
        setLoading(false);
        if(sessionErr){setErr('تعذر إتمام تسجيل الدخول، حاول مرة أخرى');return;}
