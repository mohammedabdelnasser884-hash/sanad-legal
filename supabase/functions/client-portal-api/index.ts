...
async function actionGetCases(claims: { client_id: string; tenant_id: string }) {
  const rows = await rest(
    `cases?client_id=eq.${claims.client_id}&tenant_id=eq.${claims.tenant_id}&select=id,case_number,case_type,court:court_name,status,created_at,client_name&order=created_at.desc`,
  );
...
async function actionGetCaseFees(claims: { client_id: string; tenant_id: string }, body: Record<string, string>) {
  ...
  const rows = await rest(
    `case_fees?case_id=eq.${caseId}&tenant_id=eq.${claims.tenant_id}&select=id,total_fees,paid_fees,status,last_payment_date&order=last_payment_date.asc`,
  );
  return json({ data: rows });
}
...
async function actionGetCaseSessions(claims: { client_id: string; tenant_id: string }, body: Record<string, string>) {
  ...
  const rows = await rest(
    `case_sessions?case_id=eq.${caseId}&tenant_id=eq.${claims.tenant_id}&select=id,session_date,session_hall,description,result,next_action&order=session_date.desc`,
  );
  return json({ data: rows });
}
...
async function actionGetCaseDocuments(claims: { client_id: string; tenant_id: string }, body: Record<string, string>) {
  ...
  const rows = await rest(
    `case_documents?case_id=eq.${caseId}&tenant_id=eq.${claims.tenant_id}&select=id,file_name,file_type,file_url,storage_path,created_at&order=created_at.desc`,
  );
  return json({ data: rows });
}
