...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!law.file_path) throw new Error('لا يوجد ملف PDF');
    ...
    if (dlErr || !fileBlob) throw new Error('فشل تحميل الملف');
    ...
    if (!fullText || fullText.trim().length < 20) throw new Error('تعذر استخراج نص — الملف قد يكون صور ممسوحة.');

    const articles = splitIntoArticles(fullText);
    if (articles.length === 0) throw new Error('لم يتم العثور على مواد بصيغة "مادة (رقم)". عينة:\n' + fullText.slice(0, 500));
    ...
    if (law_id) await supabase.from('laws').update({ status: 'failed', processing_error: message }).eq('id', law_id);
