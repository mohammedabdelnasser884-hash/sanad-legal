...
describe('process-law-extract — استخراج النص عبر unpdf', () => {
  it('فشل getDocumentProxy (ملف تالف) → 500 برسالة unpdf + status=failed', async () => {
    queueLawFound();
    supabaseMock.queueTable('laws', { data: null, error: null }); // update processing
    ...
    const res = await handler(req({ law_id: 'law-1' }));
    expect(res.status).toBe(500);
    expect((await res.json()).error).toBe('unpdf: فشل تحميل الملف كـ PDF');
  });
  ...
  it('مفيش أي "مادة" اتلقطت في النص → 500 برسالة فيها عينة من النص + status=failed', async () => {
    queueLawFound();
    supabaseMock.queueTable('laws', { data: null, error: null }); // update processing
    supabaseMock.queueStorageDownload('legal-library', { data: fakeBlob(), error: null });
    supabaseMock.queueTable('laws', { data: null, error: null }); // update failed
    ...
    expect(errMsg).toContain('لم يتم العثور على مواد بصيغة "مادة (رقم)"');
    expect(errMsg).toContain('هذا نص طويل');
  });
...
describe('process-law-extract — تحديث/حذف/إدراج law_articles', () => {
  it('فشل حذف المواد القديمة (delErr) → 500 برسالة الحذف + status=failed', async () => {
    queueLawFound();
    supabaseMock.queueTable('laws', { data: null, error: null }); // update processing
    supabaseMock.queueStorageDownload('legal-library', { data: fakeBlob(), error: null });
    supabaseMock.queueTable('laws', { data: null, error: null }); // update failed
    ...
    const res = await handler(req({ law_id: 'law-1' }));
    expect(res.status).toBe(500);
    expect((await res.json()).error).toBe('delete-fail');
  });

  it('فشل إدراج المواد الجديدة (insErr) → 500 برسالة الإدراج + status=failed', async () => {
    queueLawFound();
    supabaseMock.queueTable('laws', { data: null, error: null }); // update processing
    supabaseMock.queueStorageDownload('legal-library', { data: fakeBlob(), error: null });
    supabaseMock.queueTable('laws', { data: null, error: null }); // update failed
