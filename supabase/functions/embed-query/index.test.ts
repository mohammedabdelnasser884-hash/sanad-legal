...
describe('embed-query — فشل Gemini', () => {
  it('GEMINI_API_KEY مش مضاف كـ Secret → 500 من غير أي نداء fetch لـ Gemini', async () => {
    ...
    expect(res.status).toBe(500);
    const data = await res.json();
    expect(data.error).toBe('GEMINI_API_KEY غير مضاف كـ Secret في Edge Functions');
    expect(state.geminiCalls).toEqual([]);
  });

  it('Gemini بيرجع data.error → 500 برسالة "Gemini embedding error: ..."', async () => {
    state.geminiBody = { error: { message: 'quota exceeded' } };
    const res = await handler(req({ text: 'نص' }));
    expect(res.status).toBe(500);
    const data = await res.json();
    expect(data.error).toBe('Gemini embedding error: quota exceeded');
  });

  it('رد Gemini بشكل غير متوقع (من غير embedding.values array) → 500 برسالة "استجابة Gemini غير متوقعة"', async () => {
    state.geminiBody = { unexpected: 'shape' };
    const res = await handler(req({ text: 'نص' }));
    expect(res.status).toBe(500);
    const data = await res.json();
    expect(data.error).toContain('استجابة Gemini غير متوقعة');
  });
});
