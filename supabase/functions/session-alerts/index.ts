...
const runForTenant = async (office: any, type: string) => {
  ...
  const token = office.tg_daily_token;
  const chat = office.tg_daily_chat;
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: offices, error: offErr } = await supabase
      ...
      .select("tenant_id, tg_daily_token, tg_daily_chat")
      .not("tg_daily_token", "is", null)
      .not("tg_daily_chat", "is", null);
