...
**آخر تحديث:** 15 يوليو 2026 — **اكتشاف `OfflineQueueItem.id` اتصلح بعد موافقتك الصريحة. المرحلة 5 خلصت بالكامل فعليًا الآن (الجزء الوحيد المتبقي متعمَّد ومنخفض المخاطر جدًا — تفاصيله تحت).**

## ✅ إصلاح اكتشاف `OfflineQueueItem.id` — منجز، 15 يوليو 2026

**السياق:** ده الاكتشاف اللي اتوثّق في الدفعة اللي فاتت (`id` معرّف `number` بس فعليًا بيخزن `string` في عمليات UPDATE/DELETE بسبب طريقة عمل IndexedDB مع الـ keyPath). قبل ما أصلحه، اتأكدت من حاجة مهمة: **بحثت في المشروع كله عن أي استخدام لـ `OfflineQueueItem`/`__deleteOfflineItem` برّة `main.tsx`، ولقيت صفر** — الاستخدام كله محصور في 4 أسطر بس جوه نفس الملف. يعني نطاق أي تعديل هنا مضمون إنه محصور تمامًا وميأثرش على أي حاجة تانية في المشروع.

### اللي اتصلح فعليًا (`main.tsx`، كله تحديد نوع، صفر تغيير منطق):
- `OfflineQueueItem.id`: `number` → `number | string` — بيعكس الواقع الفعلي (رقم تلقائي لعمليات INSERT، id حقيقي string لعمليات UPDATE/DELETE)، مع تعليق موضّح للسبب.
- `__deleteOfflineItem`: توقيعه في `declare global` وتطبيقه الفعلي بقى بارامتر `id: number | string` بدل `number` بس — بيتوافق مع النوع الجديد فوق. `store.delete(id)` جوه IndexedDB أصلاً بيقبل `number` أو `string` كمفتاح (`IDBValidKey`)، فمفيش أي كاست أو تغيير سلوك محتاج هنا.
- `table` في `OfflineQueueItem` وكل استخدامات `op.id`/`op.table` في `__syncOfflineQueue` **فضلوا زي ما هم** (مفيش تغيير في أي منطق أو أي كاست تاني) — التعديل ده محصور في تصحيح نوع `id` بس، مش توسعة لموضوع `db.from(op.table)` الديناميكي (ده لسه بند منفصل لو حبيت نفتحه بعدين).

**فحص التوافق:** راجعت كل الأربع أماكن اللي بتستخدم `id`/`__deleteOfflineItem` (تعريف الواجهة، إعلان `Window`، تعريف الدالة، ونداءين ليها جوه `__syncOfflineQueue`) — كلهم متوافقين مع النوع الجديد. فحصت توازن الأقواس للملف كامل بعد التعديل — سليم.

**الحالة:** الاكتشاف اتقفل. **المرحلة 5 خلصت فعليًا بالكامل** — الجزء الوحيد المتبقي (متعمَّد، منخفض المخاطر) هو تضييق `OfflineQueueItem.table` نفسه وشيل الـ `as any` الأخيرة المتبقية على `db.from(op.table)` — ده بند منفصل ومحدود (مش هيتلمس من غير ما تطلبه صراحةً كخطوة جديدة).

**⚠️ نفس تحذير كل مرة:** بدون إنترنت هنا (`npm install` برضو 403)، فمقدرتش أشغّل `tsc` حقيقي. راجعت يدويًا وفحصت توازن الأقواس. **محتاج تأكيد بناء Vercel حقيقي من عندك** قبل ما نعتبر البند ده مقفول خالص.

---



## ✅ المرحلة 5 — الجزء الأخير: `window.__dbWrite` — منجز جزئيًا، 15 يوليو 2026

**السياق:** ده كان آخر بند مفتوح من المرحلة 5 (زي ما اتقال في آخر تحديث: "الباقي المفتوح الوحيد: `window.__dbWrite`/`db.from(op.table)` الديناميكي في `main.tsx`"). قسمته لجزئين بسبب اكتشاف حقيقي ظهر أثناء المراجعة (تفاصيله تحت).

### التحقق الفعلي قبل أي تعديل:
- بحثت في كل المشروع عن كل نداء فعلي لـ `window.__dbWrite(...)` — لقيت نداءين بس: `useCaseActions.ts` (3 نداءات: إدراج قضية، إدراج جلسة أولى مصاحبة، تحديث قضية) و`useClientActions.ts` (نداء واحد: إدراج موكل). أسماء الجداول الفعلية المستخدمة: **`cases`، `case_sessions`، `clients`** بس — مفيش أي جدول رابع.
- اتأكدت إن التلاتة دول عندهم `id: string` و`updated_at: string | null` حقيقيين في `database.types.ts` (مطلوبين للـ Optimistic Locking).

### اللي اتصلح فعليًا (`main.tsx`، كله تحديد نوع، صفر تغيير منطق):
- نوع جديد `DbWriteTable = 'clients' | 'cases' | 'case_sessions'` — بيوصف بالظبط الجداول اللي بتتكتب عن طريق `__dbWrite`.
- توقيع `window.__dbWrite` في `declare global` بقى **Generic** (`<T extends DbWriteTable>`)، بنفس أسلوب `dynFrom` في `useAdminBackup.ts` و`safeUpdate` في `utils.ts` من دفعات سابقة:
  - `table: T` بدل `string` — بيتحقق وقت الكتابة إن اسم الجدول حقيقي وموجود فعلاً.
  - `id: string`، `knownUpdatedAt: string | null` بدل `any` — نفس الشكل اللي كل الاستدعاءات الفعلية بتبعته.
  - `data?: Record<string, unknown>` — **فضلت عن قصد** (مش `Insert`/`Update` الصارم): نداء واحد فعلي (حفظ جلسة أولى مصاحبة لقضية أوفلاين) بيبعت حقل sentinel مؤقت (`_offlineCaseTitle`) مش عمود DB حقيقي، بيتحذف قبل الإدراج الفعلي وقت المزامنة. ربطه بنوع صارم كان هيرفضه غلط رغم إنه سلوك مقصود وشغّال.
  - القيمة المرجعة `data?: Partial<Row> | null` — `Partial` مش `Row` الكامل، لأن مسار الـ UPDATE بيرجّع فعليًا عمود `updated_at` بس (مش الصف كامل)، بعكس الـ INSERT اللي بيرجّع الصف كامل. `Partial` بتغطي الحالتين من غير ما تدّعي شكل مش حقيقي.
- تطبيق الدالة نفسه (`window.__dbWrite = async function <T extends DbWriteTable>(...)`) بقى بنفس الأنواع دي بالكامل — شلت الـ `(window as any)` والـ `: any` من الباراميترات، وشلت الكاست `as any` اللي كان على `db.from(table)` في مسار الـ Optimistic Locking (بقى مش محتاجه بعد ما `table` بقى Generic حقيقي).
- الكاستات الوحيدة المتبقية جوه التطبيق (`data as Database['public']['Tables'][T]['Insert']` و`... ['Update']`، و`id as string`) — كاستات ضيقة موثّقة بتحافظ 100% على نفس السلوك، بنفس منطق `useAdminBackup.ts`: الجدول نفسه بقى متحقق بالكامل، والكاست الوحيد الباقي محصور في شكل البيانات نفسها.

### ⚠️ اكتشاف جديد — مؤجل عمدًا لحد قرارك الصريح:
أثناء مراجعة `__syncOfflineQueue` (نفس مسار الكتابة، بس للعمليات المؤجلة أوفلاين) لقيت تضارب نوع حقيقي موجود من قبل التعديل ده، مش سببه: `OfflineQueueItem.id` معرّف `number` (مفتاح IndexedDB التلقائي)، لكن IndexedDB بيستخدم أي خاصية بنفس اسم الـ `keyPath` ('id') لو كانت معرّفة كـ *قيمة حقيقية* بدل ما يولّد رقم — يعني في عمليات UPDATE/DELETE، القيمة المخزّنة فعليًا في `id` هي الـ id الحقيقي (string) بتاع الصف، مش رقم. ده نفس فصيلة اكتشافات `case_type`/`next_session` السابقة (فجوة نوع كانت مستخبية وراء التخصيص القديم).

**عشان كده مبضيّقتش** `OfflineQueueItem.table` (فضل `string`) ولا لمست الـ `as any` الموجودة على `db.from(op.table)` جوه `__syncOfflineQueue` — تضييقهم كان هيكشف التعارض ده في `.eq('id', op.id)` ويحتاج مراجعة منفصلة (هل نغيّر نوع `id` لـ `string | number`؟ نفصل حقل تاني لقيمة الـ record id الحقيقية بدل ما نستخدم نفس `id` لغرضين؟). **مش هغيّر أي حاجة هنا من غير موافقتك الصريحة** — الكود شغّال زي ما هو تمامًا، الملاحظة دي توثيقية بس.

**الحالة:** جزء `window.__dbWrite` خلص بالكامل. الباقي المفتوح: قرارك في اكتشاف `OfflineQueueItem.id` فوق (اختياري، مش لازم نتحرك فيه)، ولو قررت نكمله هو آخر حاجة متبقية فعليًا في `main.tsx`/`App.tsx` بالكامل.

**⚠️ نفس تحذير الفحص من كل مرة:** البيئة هنا بدون إنترنت (`npm install` رجع 403 برضو)، فمقدرتش أشغّل `tsc` حقيقي. راجعت السطور يدويًا (كل نداء فعلي لـ `__dbWrite`، وتوافق أنواع `id`/`updated_at` مع الجداول التلاتة) وفحصت توازن الأقواس للملف كامل. **محتاج تأكيد بناء Vercel حقيقي من عندك** قبل ما نعتبر البند ده مقفول.

---


بعتّلي 3 أخطاء `tsc` حقيقية من بناء Vercel فعلي (مش فحص أوفلاين) — وده بالظبط الفيصل اللي كنا مستنينه. الأخطاء الثلاثة كانت من نفس الفصيلة: **أنواع بارامترات معرّفة في ملفات تانية (مش اللي اتعدلت) كانت أوسع من الاستخدام الفعلي ليها**، وكانت مستخبية وراء الـ `any` القديم قبل ما نحدد type لـ `profile`/`authUser`/`selectedClient`/`setTab`.

### 1) `setAuthUser: (user: unknown) => void` في `useCaseActions.ts`
كانت متعرّفة `unknown` (تقبل أي حاجة)، لكن الاستخدام الفعلي جوه الملف بينادي بس `setAuthUser(null)`. ضيّقتها لـ `(user: { id: string; email?: string | null } | null) => void` — نفس الشكل اللي `authUser` بقى عليه فعليًا في `App.tsx`.

### 2) `setSelectedClient` في `useClientActions.ts`
**ده كان بالفعل اتصلح في دفعة "جزء 2ب" اللي فاتت** (ضيّقته من `Dispatch<SetStateAction<ClientRow|null>>` لـ `(v: ClientRow | null) => void`) — لكن ظاهر من رسالة الخطأ إن النسخة اللي اتبنت على Vercel كانت لسه بالنوع القديم. **محتمل الملف ده تحديدًا ماتحدّثش وقت الدمج.** اتأكدت إنه في الزيب الجديد ده بالنوع الصح.

### 3) `setTab: (t: string) => void` في `DashboardTab.tsx`
لما ضيّقنا `setTab` في `App.tsx` لـ `TabName` بدل `string`، طلع إن `DashboardTab.tsx` كانت بتعلن `setTab` بتاعها كـ `(t: string) => void` (أوسع). كل الاستخدامات الفعلية جواها (`setTab('reminders')`, `setTab('calendar')`) قيم حقيقية من `TabName` — فضيّقت النوع هناك كمان لـ `(t: TabName) => void` (استوردت `TabName` من `../../useNavigation`).

**الحالة:** الثلاث حالات كلها تصحيح دقة نوع بس (النوع القديم كان أوسع من الاستخدام الحقيقي) — **صفر تغيير منطق**. الزيب المرفق ده فيه **كل** التعديلات (2أ + 2ب + إصلاح البناء ده) مجمّعة مع بعض، عشان نتجنب مشكلة "ملف اتنسي وقت الدمج" زي اللي حصلت مع `useClientActions.ts`.

**⚠️ مهم جدًا:** لما تدمج الزيب ده، تأكد إنه بيستبدل **كل** الملفات الأربعة (`App.tsx`, `useCaseActions.ts`, `useClientActions.ts`, `DashboardTab.tsx`) مش بس اللي فاكرها اتغيرت — عشان مانوقعش في نفس مشكلة الملف المنسي تاني.

---



## ✅ المرحلة 5 — جزء 2ب (selectedCase/selectedClient) — منجز، 15 يوليو 2026

**قررت (بناءً على طلبك "اعمل اللي انت شايفه صح") أتحقق الأول من مدى خطورة الاكتشاف قبل ما أختار طريقة الحل، بدل ما أفترض:**

### التحقق الفعلي (بحث شامل في الكود، مش افتراض):
- **جانب القضايا (`year`/`session_time` الناقصين في `SearchCaseResult`):** بحثت في كل المشروع — الحقلين دول **مش بيتقروا من `caseData` في أي مكان خالص** (لا في `CaseDetailView.tsx` ولا `useCaseDetailActions.ts`). يعني الفجوة دي **خاملة تمامًا (inert)** — موجودة في النوع بس مالهاش أي أثر فعلي وقت التشغيل.
- **جانب الموكلين (`SearchClientResult`):** بحثت في `ClientDetailModal.tsx` ولقيت إنه فعليًا بيقرا 4 حقول مش موجودة في `SearchClientResult`: `contact_info`, `cr_number`, `notes`, `type`. يعني **لو حد فتح موكل من نتيجة البحث السريع (مش من قائمة الموكلين العادية)، الحقول دي هتبان فاضية في شاشة تفاصيل الموكل** — دي فجوة بيانات حقيقية وموجودة من الأول، مش حاجة سببها التعديل الحالي (كانت مستخبية وراء الـ `any`).

### القرار اللي اتخذته:
- **مبنيتش نوع `selectedCase`/`selectedClient` كـ union واسع** (`MappedCase | SearchCaseResult`) لأن ده كان هيتعارض مع النوع اللي `useCaseActions.ts` أصلاً بيتوقعه (`MappedCase | null` بالظبط، من مرحلة سابقة اتفقنا عليها ومتأكدة). بدل كده:
  - `selectedCase` → `MappedCase | null`، و`selectedClient` → `MappedClient | null` (زي ما كان مخطط له من الأول).
  - في المكان الوحيد اللي بيوصل منه شكل أضيق فعليًا (نتيجة بحث UniversalSearchModal)، استخدمت **كاست ضيق موثّق** (`c as MappedCase` / `c as MappedClient`) — بيحافظ 100% على نفس السلوك الحالي بالظبط (زي ما كان وقت ما كانت `any`)، من غير أي تغيير منطق. حطيت تعليق فوق الكود يشرح الفجوة بالظبط.
  - **مفيش أي إصلاح سلوكي** لفجوة بيانات الموكل (زي "هات بيانات الموكل كاملة أول ما يتفتح من البحث") — ده قرار منطقي حقيقي محتاج موافقتك الصريحة زي أي حاجة تانية من النوع ده، فسبته زي ما هو وموثّق هنا لو حبيت نرجعله لاحقًا.

### أماكن تانية اتصلحت في نفس الدفعة (كلها تحديد نوع بس):
- إغلاقات (closures) داخل `.filter`/`.find`/`.map` كانت بتفقد الـ narrowing بتاع TypeScript لـ `selectedCase`/`selectedClient` (لأن TypeScript مابيوصلش الـ narrowing جوه دالة closure) — استخدمت `?.` (optional chaining) بدل `.` في 3 أماكن (`selectedClient?.id`, `selectedCase?.client_id`, `selectedCase?.id`). ده **صفر تغيير سلوك** لأن القيمة مضمونة مش null في كل الحالات دي (محمية بشرط `&&` قبلها في نفس الرندر)، بس التعليم بالـ `?.` كان لازم عشان الكومبايلر يقبله.
- `setSelectedCase((p: any) => ({...p, status: newStatus}))` → بقت `(p) => ({...p, status: newStatus} as MappedCase)` — كاست ضيق بيحافظ 100% على نفس السلوك (بما فيه حالة `p===null` النظرية اللي مبتحصلش عمليًا لأن `onUpdate` بس بيتنادى لما `selectedCase` أصلاً مش null).
- **`src/hooks/useClientActions.ts`:** سطر واحد بس — `setSelectedClient` كان معرّف `Dispatch<SetStateAction<ClientRow | null>>` (بيسمح بدالة تحديث كمان)، لكن اتأكدت إن كل استخدام فعلي ليه جوه الملف بينادي بس بـ `null` مباشرة (مفيش أي مكان بيبعتله دالة). ضيّقت النوع لـ `(v: ClientRow | null) => void` عشان يعكس الاستخدام الحقيقي بالظبط — تصحيح دقة نوع، مش تغيير سلوك.

**فحص التوافق (يدوي، البيئة بدون إنترنت زي كل مرة):**
- راجعت كل مستهلكي `setSelectedCase`/`setSelectedClient` في المشروع (`DashboardTab.tsx`, `CasesTab.tsx`, `ClientsTab.tsx`, `AILegalAssistant.tsx` — ده آخر واحد عنده `selectedCase`/`setSelectedCase` محلي خاص بيه من `useAIAssistant`، مش نفس الـ state بتاع `App.tsx`) وأكدت التوافق مع الأنواع الجديدة.
- **محتاج تأكيد بناء Vercel حقيقي من عندك** قبل ما نعتبر المرحلة 5 كلها مقفولة.

**الحالة: المرحلة 5 خلصت بالكامل (2أ + 2ب).** الباقي المفتوح الوحيد فيها: `window.__dbWrite`/`db.from(op.table)` الديناميكي في `main.tsx` (مسار الكتابة العام، أخطر حاجة في المشروع) — لسه متأجل زي ما اتفقنا، محتاج دفعة منفصلة لوحده.

---



## ✅ المرحلة 5 — جزء 2أ (من الجزء المؤجل: profile/authUser/selectedCase/selectedClient/deleteConfirm) — منجز جزئيًا، 15 يوليو 2026

**السياق:** الجزء ده كان أخطر جزء متبقي في المشروع (زي ما اتفقنا في نهاية جزء 1)، فقسمته لحتتين: **2أ** (الحالات الآمنة اللي اتأكدت إن كل حاجة تستهلكها في المشروع أصلاً متجهزة لها من مراحل سابقة) و**2ب** (selectedCase/selectedClient — اتأجلت، فيها اكتشاف محتاج قرارك، تفاصيله تحت).

### اللي اتصلح فعليًا في 2أ (كله تحديد نوع، صفر تغيير منطق):

**`src/App.tsx`:**
- `profile` → `ProfileRow | null` بدل `any`. اتأكدت قبل التعديل إن **كل** حاجة بتستقبل `profile` في المشروع (useAppData, useHealthMonitor, useDashboardFeed, useTelegramAlerts, useAutoLogout, AppHeader, DashboardTab, CaseDetailView, AILegalAssistant, SettingsPage, AdminPanel) أصلاً بتتوقع `ProfileRow | null` بالظبط من مراحل سابقة — يعني التعديل ده بس بيقفل فجوة كانت موجودة، مش بيكسر حاجة.
- `authUser` → `{ id: string; email?: string | null } | null` بدل `any` (نفس الشكل اللي `LoginScreen` أصلاً بيتوقعه في `onLogin`، وهو الحقل الوحيد (`id`) اللي بيتقرا من `authUser` في كل المشروع).
- `loadProfile(user: any)` → نفس النوع اللي فوق. بيتنادى بـ `session.user` (النوع الحقيقي من supabase-js) وبـ `u` من `LoginScreen.onLogin` — الاتنين متوافقين هيكليًا مع النوع الجديد.
- `onLogin: (u: any) => loadProfile(u)` → شالت الـ `any`، الكومبايلر بيستنتج نوع `u` تلقائيًا من `LoginScreenProps.onLogin` المُعرَّف أصلاً.
- `db.auth.getSession().then(({data:{session}}: any) => ...)` و`onAuthStateChange((_event: any, session: any) => ...)` → شالت الـ `any` بالكامل من غير ما أحط أنواع بديلة يدوي — أنواع supabase-js نفسها (اللي الـ `db` عميل متولّد منها بـ `createClient<Database>`) بتوصف الباراميترات دي تلقائيًا لما الـ `any` تتشال، فمفيش أي احتمال اختلاف مع النوع الحقيقي بتاع المكتبة.
- `deleteConfirm` state → `DeleteConfirmState | null` بدل `any` (النوع ده أصلاً موجود ومُستخدم جوه `useCaseActions.ts`/`useClientActions.ts` — نفس الشكل بالظبط في الاتنين، فصدّرته من `useCaseActions.ts` (كلمة `export` بس، بدون أي تغيير في محتواه) واستوردته في `App.tsx`). دالة `setDeleteConfirm` الداخلية اتحدّث بنفس النوع.
- `setTab`: شالت الكاست `nav.navigateTo(newTab as any)` خالص — بقى `newTab: TabName` (النوع الحقيقي بتاع `useNavigation.ts`). عشان ده يشتغل من غير كسر أي استدعاء، الأربع عناصر في مصفوفة أزرار "المزيد" (`clients`/`documents`/`fees`/`admin`) اتأكدت إن قيمة `tab` فيهم بقت `as TabName` (نفس القيم النصية الأربعة بالظبط زي ما كانت، بس التنويت بقى دقيق بدل ما يتستنتج `string` عام).

**`src/hooks/useCaseActions.ts`:**
- تعديل سطر واحد بس: `interface DeleteConfirmState` بقت `export interface DeleteConfirmState` — عشان `App.tsx` يقدر يستخدمها. مفيش أي تغيير في محتوى الـ interface نفسها.

**فحص التوافق (يدوي، مفصّل قبل التسليم — البيئة هنا بدون إنترنت زي الجولة اللي فاتت فمقدرتش أشغّل `tsc --strict` حقيقي):**
- راجعت كل الملفات المستهلكة لـ `profile` في المشروع كله (9 ملفات) وأكدت إنها كلها بتتوقع `ProfileRow | null` بالظبط.
- راجعت `useCaseActions.ts`/`useClientActions.ts` وأكدت إن الأنواع اللي بيتوقعوها لـ `setProfile`/`setAuthUser`/`setDeleteConfirm` متطابقة تمامًا مع اللي بقى موجود دلوقتي في `App.tsx`.
- راجعت `DeleteConfirmModal.tsx` وأكدت إن كل حقل من `DeleteConfirmState` (title/itemName/itemType/mode/onConfirm) متوافق مع الـ props بتاعته.
- **محتاج تأكيد بناء Vercel حقيقي من عندك** قبل ما نعتبر 2أ مقفولة نهائيًا — زي كل مرة، ده الفيصل الحقيقي مش مراجعتي اليدوية.

---

## ⚠️ اكتشاف جديد — محتاج قرارك قبل ما نكمل 2ب (selectedCase/selectedClient)

وأنا براجع تحديد نوع `selectedCase`/`selectedClient` (لـ `MappedCase | null` و`MappedClient | null`)، لقيت إن ده هيكسر البناء فعليًا — مش مجرد تنظيف نوع بسيط زي الباقي. السبب:

- `UniversalSearchModal.tsx` بيسمح إنه يبعت لـ `onOpenCase` قيمة من نوع **`SearchCaseResult`** (شكل نتيجة البحث، مُعرَّف داخل الملف نفسه) — وده شكل **ناقص** مقارنة بـ `MappedCase`: ناقصه حقلين حقيقيين (`year`, `session_time`) موجودين في `MappedCase` وبيتفترض وجودهم في `CaseDetailView`/`useCaseDetailActions` وأي حتة تانية بتستخدم `selectedCase`.
- نفس الحكاية بالظبط مع `onOpenClient` → `SearchClientResult`: شكل مُختصَر (id/full_name/phone/email/national_id بس) مقارنة بـ `MappedClient` (= `ClientRow` الكاملة، عشرات الأعمدة).

يعني لو حد فتح قضية أو موكل من نتيجة البحث السريع (مش من قائمة القضايا/الموكلين العادية)، الـ state اللي بيتخزن فعليًا وقت التشغيل شكله أقل من اللي المفروض. ده نفس فصيلة الباگات اللي اكتشفناها قبل كده (`case_type`, `.date`) — بس هنا الاكتشاف نفسه جه من التحقق الآلي مش يدوي.

**مهم:** الـ any الحالي كان بيغطي الفجوة دي — يعني ده مش باگ جديد حصل بسببي، ده باگ/فجوة نوع كانت موجودة أصلاً ومستخبية وراء الـ `any`. مفيش أي تغيير سلوك حصل، وده لسه معلّق ومحتاج قرارك.

**الخيارات المتاحة (قولّي تحب نمشي بإيه، أو لو عندك اقتراح تاني):**
1. **نوسّع نوع `selectedCase`/`selectedClient`** ليشمل الاتنين (`MappedCase | SearchCaseResult | null` مثلاً) — ده بيوصف الواقع الحالي بالظبط من غير أي تغيير سلوك، بس بيعني إن أي حتة بتستخدم `selectedCase.year` أو `.session_time` لازم تتعامل معاهم كـ optional/ممكن يكونوا مش موجودين (لو فعلاً بيتقروا في مكان ما).
2. **نتحقق الأول هل `year`/`session_time` بيتقروا فعليًا من `caseData` في أي مكان** (زي `CaseDetailView` أو `useCaseDetailActions`) — لو مش بيتقروا خالص، الفجوة نظرية بس مش بتأثر عمليًا، ونقدر نوثقها ونمشي بالخيار 1 براحة.
3. **نسيب `selectedCase`/`selectedClient` زي ما هما (`any`) دلوقتي** ونقفل المرحلة 5 هنا، ونرجعلها لاحقًا لو حبيت.

**ملاحظة:** مفيش أي تغيير منطق في أي حاجة في الجزء ده. الباقي المفتوح: قرارك في الاكتشاف فوق، وبعده لسه فاضل `db.auth.getSession()`/`onAuthStateChange` callbacks (خلصوا فعليًا في 2أ فوق) و`window.__dbWrite`/`db.from(op.table)` الديناميكي في `main.tsx` (مسار الكتابة العام، أخطر حاجة في المشروع - لسه متأجلة زي ما اتفقنا).
